...
function createTextTexture(gl, text, font = 'bold 30px monospace', color = 'black', Texture) {
...
class Title {
  constructor({ gl, plane, renderer, text, textColor = '#545050', font = '30px sans-serif', ogl }) {
    autoBind(this);
    this.gl = gl;
    this.plane = plane;
  ...
  createMesh() {
    const { texture, width, height } = createTextTexture(this.gl, this.text, this.font, this.textColor, this.ogl.Texture);
...
class CircularGalleryApp {
  ...
  createMedias(items, bend, textColor, borderRadius, font) {
    ...
    this.mediasImages = galleryItems.concat(galleryItems); // Duplicate for infinity scroll
